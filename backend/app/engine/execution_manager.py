...
class ExecutionManager:
    def __init__(self, broker, state_manager, event_bus, reconciliation=None, max_retries: int = 3, base_backoff: float = 1.0):
        self.broker = broker
    ...
    async def execute_order(self, request: dict[str, Any]) -> ExecutionResult:
        ...
        if await self.state_manager.has_idempotency_key(key):
            ...
            return ExecutionResult(order_id=None, filled_qty=0, avg_price=None, state=OrderState.ABORTED)

        await self.state_manager.add_idempotency_key(key)
        exec_ctx = {"state": OrderState.NEW, "order_id": None, "filled_qty": 0, "avg_price": None}

        for attempt in range(1, self.max_retries + 1):
            self._transition(exec_ctx, OrderState.PLACING)
            try:
                resp = await self.broker.place_order(symbol=symbol, side=side, quantity=qty)
                ...
                if not oid:
                    ...
                    if fill_state == "PARTIAL_FILL":
                        ...
                        return ExecutionResult(oid, fqty, avg, OrderState.ORDER_UNCERTAIN, True)
                ...
                logger.error("execute_order broker exception attempt=%d/%d symbol=%s side=%s err=%s", attempt, self.max_retries, symbol, side, exc)
                await self._mark_uncertain(exec_ctx, reason="broker_api_error")
                return ExecutionResult(exec_ctx.get("order_id"), exec_ctx["filled_qty"], exec_ctx["avg_price"], OrderState.ORDER_UNCERTAIN, True)
        ...
        return ExecutionResult(exec_ctx.get("order_id"), exec_ctx["filled_qty"], exec_ctx["avg_price"], OrderState.ABORTED, False)
    ...
    async def _mark_uncertain(self, exec_ctx: dict, reason: str) -> None:
        ...
        await self.state_manager.update(trading_enabled=False, last_order_failed=True, last_risk_breach="execution_uncertain")
        await self.event_bus.publish("EXECUTION_UNCERTAIN", {"order_id": exec_ctx.get("order_id"), "reason": reason})
    ...
    def _transition(self, exec_ctx: dict, new_state: OrderState) -> None:
        ...
        logger.info("Order state transition %s -> %s order=%s", old, new_state, exec_ctx.get("order_id"))
