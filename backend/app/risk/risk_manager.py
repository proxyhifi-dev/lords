...
class RiskManager:
    """
    Optional standalone risk gatekeeper.

    Listens to SIGNAL events and re-publishes them as RISK_APPROVED
    (or RISK_BLOCKED).  Wire into the scheduler if you want to split
    the risk check out of TradingEngine.
    """
        ...
        async for event in self.event_bus.iter_events(queue):
            ...
            if state.trade_count >= settings.max_trades:
                await self.event_bus.publish(
                    "RISK_BLOCKED", {"reason": "max_trades_reached"}
                )
                continue
            ...
            if state.daily_pnl <= -abs(settings.max_daily_loss):
                ...
                await self.event_bus.publish(
                    "RISK_BLOCKED", {"reason": "max_daily_loss"}
                )
                continue

            if not state.trading_enabled:
                await self.event_bus.publish(
                    "RISK_BLOCKED", {"reason": "trading_disabled"}
                )
                continue
