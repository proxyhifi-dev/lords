...
from app.schemas import AutoSliceOrderRequest, MultiLegOrderRequest, MultiOrderRequest, OrderRequest
...
class OrderService:
    ...
    async def place_auto_slice_order(self, payload: AutoSliceOrderRequest | dict) -> dict:
        ...
        return await self.client.request(
            "POST",
            "/orders/sync",
            data=order.model_dump(exclude_none=True) | {"autoslice": True},
        )

    @staticmethod
    def _validate_tick_and_quantity(order: OrderRequest) -> None:
        # Placeholder: connect this to exchange instrument master for exact tick/lot checks.
        if order.qty <= 0:
