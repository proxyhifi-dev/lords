from __future__ import annotations

import asyncio
from collections import defaultdict
from dataclasses import asdict, dataclass, field
from time import time
from typing import Any, AsyncIterator


@dataclass(slots=True)
class Event:
    type: str
    payload: dict[str, Any] = field(default_factory=dict)
    timestamp: float = field(default_factory=time)

    def to_dict(self) -> dict[str, Any]:
        return asdict(self)


class EventBus:
    """Asynchronous in-process message bus built on asyncio.Queue."""

    def __init__(self, maxsize: int = 10_000) -> None:
        self._ingress: asyncio.Queue[Event] = asyncio.Queue(maxsize=maxsize)
        self._subscribers: dict[str, list[asyncio.Queue[Event]]] = defaultdict(list)
        self._all_subscribers: list[asyncio.Queue[Event]] = []
        self._dispatcher_task: asyncio.Task[None] | None = None
        self._running = asyncio.Event()

    async def start(self) -> None:
        if self._dispatcher_task and not self._dispatcher_task.done():
            return
        self._running.set()
        self._dispatcher_task = asyncio.create_task(self._dispatch_loop(), name="eventbus-dispatcher")

    async def stop(self) -> None:
        self._running.clear()
        if self._dispatcher_task:
            self._dispatcher_task.cancel()
            try:
                await self._dispatcher_task
            except asyncio.CancelledError:
                pass

    async def publish(self, event_type: str, payload: dict[str, Any] | None = None) -> None:
        await self._ingress.put(Event(type=event_type, payload=payload or {}))

    def subscribe(self, event_type: str | None = None, maxsize: int = 2000) -> asyncio.Queue[Event]:
        queue: asyncio.Queue[Event] = asyncio.Queue(maxsize=maxsize)
        if event_type is None:
            self._all_subscribers.append(queue)
        else:
            self._subscribers[event_type].append(queue)
        return queue

    async def iter_events(self, queue: asyncio.Queue[Event]) -> AsyncIterator[Event]:
        while True:
            yield await queue.get()

    async def _dispatch_loop(self) -> None:
        while self._running.is_set():
            event = await self._ingress.get()
            for queue in self._subscribers.get(event.type, []):
                await queue.put(event)
            for queue in self._all_subscribers:
                await queue.put(event)
